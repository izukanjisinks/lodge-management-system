...
// LeaveRequestAssignedTemplate — kept for workflow engine compatibility
func LeaveRequestAssignedTemplate(requesterName, leaveType string, days int, startDate, endDate string) string {
	header := headerGradient(colorPrimary, colorPrimaryLight)
	body := fmt.Sprintf(`
              <p style="margin-top:0;">Hello,</p>
              <p>A request has been assigned to you for review.</p>
              %s
              <p>Please log in to the Lodge Management System to review and take action.</p>
              %s
              %s`,
		infoTable(colorInfoBox, colorInfoBorder,
			infoRow("Requested by:", requesterName),
			infoRow("Type:", leaveType),
			infoRow("Duration:", fmt.Sprintf("%d days", days)),
			infoRow("Period:", fmt.Sprintf("%s to %s", startDate, endDate)),
		),
		loginButton(),
		signature(),
	)
	return emailWrapper("Request Assigned for Review", header, body)
}

// LeaveRequestApprovedTemplate — kept for workflow engine compatibility
func LeaveRequestApprovedTemplate(employeeName, leaveType string, days int, startDate, endDate, approverName string) string {
	...
	body := fmt.Sprintf(`
		...
		infoTable(colorSuccessBox, colorSuccessBorder,
			...
			infoRow("Duration:", fmt.Sprintf("%d days", days)),
			infoRow("Period:", fmt.Sprintf("%s to %s", startDate, endDate)),
			infoRow("Approved by:", approverName),
		),
		loginButton(),
		signature(),
	)
	return emailWrapper("Request Approved", header, body)
}

// LeaveRequestRejectedTemplate — kept for workflow engine compatibility
func LeaveRequestRejectedTemplate(employeeName, leaveType string, days int, startDate, endDate, reviewerName, reason string) string {
	...
	body := fmt.Sprintf(`
              <p style="margin-top:0;">Hello %s,</p>
              <p>We regret to inform you that your request has not been approved.</p>
              %s
		...
		infoTable(colorWarningBox, colorWarningBorder,
			append(
				...
				func() []string {
					if reason != "" {
						return []string{infoRow("Reason:", reason)}
					}
					return nil
				}()...,
			)...,
	...
	return emailWrapper("Request Not Approved", header, body)
}

// WelcomeEmployeeTemplate — kept for backward compatibility, delegates to WelcomeUserTemplate
func WelcomeEmployeeTemplate(firstName, lastName, userEmail, password string) string {
	return WelcomeUserTemplate(firstName+" "+lastName, userEmail, password)
}

// PayslipReadyTemplate — kept for backward compatibility
func PayslipReadyTemplate(firstName, period string) string {
	...
	body := fmt.Sprintf(`
              ...
              <p>Log in to the Lodge Management portal to view your full payslip breakdown.</p>
              %s
              %s`,
		firstName, period,
		loginButton(),
	...
	return emailWrapper("Your Payslip is Ready", header, body)
}
